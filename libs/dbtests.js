...
findTagForDevice(aAddress64, aTagNameAddress).then(function(deviceConfig) {
            if (deviceConfig) {
        ...
        update.$inc['summary.sum'] = aValue;
        update.$max['summary.max'] = aValue;
        update.$min['summary.min'] = aValue;
        update.$inc['hours.' + dateTime.getHours() + '.summary.count'] = 1;
        update.$inc['hours.' + dateTime.getHours() + '.summary.sum'] = aValue;
        update.$max['hours.' + dateTime.getHours() + '.summary.max'] = aValue;
        update.$min['hours.' + dateTime.getHours() + '.summary.min'] = aValue;
        update.$inc['hours.' + dateTime.getHours() + '.minutes.' + dateTime.getMinutes() + '.summary.count'] = 1;
        update.$inc['hours.' + dateTime.getHours() + '.minutes.' + dateTime.getMinutes() + '.summary.sum'] = aValue;
        update.$max['hours.' + dateTime.getHours() + '.minutes.' + dateTime.getMinutes() + '.summary.max'] = aValue;
        update.$min['hours.' + dateTime.getHours() + '.minutes.' + dateTime.getMinutes() + '.summary.min'] = aValue;
        deviceTimeseries.update({
            ...
            if (err) {
                debug.log("doTestDBUpdate()-" + err);
                reject(err);
            } else {
                //log.debug("doTestDBUpdate - update result-" + JSON.stringify(result));
                resolve({
...
var tsEntry = JSON.parse('{"timestamp":"2017-03-18T01:30:50.783Z","type":146,"address64":"0013a20040485779","digitalSamples":{"DIO2":1},"analogSamples":{"AD1":1500,"SUPPLY":3.275}}');
